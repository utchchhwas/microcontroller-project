...
const int THRESHOLD_FORCE = 10;
const long COOLDOWN_TIME = 5000;
const long THRESHOLD_TIME = 5000;

int currState;
long startTime, cooldownStartTime;
...
{  
    ...
    if (currState == START_STATE) {
        ...
        if (fsrForce >= THRESHOLD_FORCE) {
            ...
            currState = COOLDOWN_STATE;
            cooldownStartTime = millis();
            long currTime = millis();
            elapsedTime = currTime - startTime;
        }
    }
    else if (currState == COOLDOWN_STATE) {
        if (fsrForce >= THRESHOLD_FORCE) {
            currState = ACTIVE_STATE;
            long currTime = millis();
            elapsedTime = currTime - startTime;
            ...
            long currTime = millis();
            long waitingTime = currTime - cooldownStartTime;
            if (waitingTime >= COOLDOWN_TIME) {
                currState = START_STATE;
            }
            else {
                long currTime = millis();
    ...
    lcdI2C.clear();
    String line1 = String("Time: ") + String(elapsedTimeSeconds) + String(" s");
    lcdI2C.print(line1);
